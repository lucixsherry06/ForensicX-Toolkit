...
app = typer.Typer(help="🔍 Digital Forensics Toolkit — use --help with any command", no_args_is_help=True)
...
EXAMPLES:
  ...
  python main.py recovery run C:\\recyclebin_dump
...
def cmd_image_clear(image_path: str):
    ...
    p = check_file_exists(image_path, "Image")
    safe_exec(clear_image_metadata, str(p))
...
def cmd_image_extract(image_path: str):
    ...
    p = check_file_exists(image_path, "Image")
    safe_exec(extract_image_metadata, str(p))


@metadata_app.command("pdf-clear")
def cmd_pdf_clear(pdf_path: str):
    """Remove all metadata from a PDF."""
    p = check_file_exists(pdf_path, "PDF")
    safe_exec(clear_pdf_metadata, str(p))


@metadata_app.command("docx-extract")
def cmd_docx_extract(docx_path: str):
    """Extract metadata from a DOCX file."""
    p = check_file_exists(docx_path, "DOCX")
    safe_exec(extract_docx_metadata, str(p))
...
def cmd_recovery(source_dir: str):
    ...
    p = check_file_exists(source_dir, "Source directory")
    safe_exec(recover_files, str(p))
...
def cmd_stego_encode(
    ...
    typer.secho(f"[SUCCESS] Output saved as {output_path}", fg=typer.colors.GREEN)
...
def cmd_stego_decode(image_path: str):
    ...
    p = check_file_exists(image_path, "Image")
    msg = safe_exec(decode_message, str(p))
